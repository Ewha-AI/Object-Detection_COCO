...
##        print('out_o: ',out_o.shape)
        weight = weight + self.weight_diff
        out_l = F.conv2d(x, weight,padding=(4,4),dilation=(4,4),stride=self.stride,groups=self.groups)
        weight = weight + self.weight_diff
        out_m = F.conv2d(x, weight,padding=(10,10),dilation=(10,10),stride=self.stride,groups=self.groups)
