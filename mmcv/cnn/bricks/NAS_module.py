...
#        self.alpha = torch.nn.Parameter(1e-3*torch.randn(2,self.num_ops).cuda(),requires_grad=True)
    ...
    def forward(self,x):
        ...
        weight_k3 = self._get_weight(self.weight)
#        weight_k5_ = self._get_weight(self.weight_k5)
...
#        out_52 = self.op52(x)
        #dil_conv_3x3_1
        out_31 = F.conv2d(x, weight_k3,padding=(1,1),dilation=(1,1),stride=self.stride,groups=self.groups)
        #dil_conv_3x3_2
        weight_k3 = weight_k3 + self.weight_diff
        out_32 = F.conv2d(x, weight_k3,padding=(2,2),dilation=(2,2),stride=self.stride,groups=self.groups)
        
        #dil_conv_3x3_3
#        weight_k3 = weight_k3 + self.weight_diff
#        out_33 = F.conv2d(x, weight_k3,padding=(3,3),dilation=(3,3),stride=self.stride,groups=self.groups)
#        weight_k3 = weight_k3 + self.weight_diff
...
#                out_7 = w*out_53
#        out = out_31 + out_32
        out = out_2 + out_3 + out_5 + out_6 #+ out_6
